...
// ═══════════════════════════════════════════════════════════════════════════
// OTA wire-contract FFI (rust/ota) — device side of the Zafu/Zigner
// firmware/module OTA. Verified stream scan -> stage, and device-signed
// result/status emission. See docs/design/zafu-ota-wire-freeze.md.
// ═══════════════════════════════════════════════════════════════════════════

/// Summary of a verified `ur:zafu-stream` manifest, for the confirm screen.
pub struct OtaStreamManifestSummary {
    pub version: String,
    pub board: String,
    ...
    pub req_id_hex: String,
}

/// Decoded + signature-verified `ur:zafu-result`.
pub struct OtaResultFFI {
    pub fw_version: String,
    pub success: bool,
    pub slot: u8, // b'A' | b'B'
    pub req_id: Vec<u8>,
    pub zid_pubkey: Vec<u8>,
    pub result_sig: Vec<u8>,
}

/// Decoded + signature-verified `ur:zafu-status`.
pub struct OtaStatusFFI {
    pub fw_version: String,
    pub slot: u8,
    pub successful_boot: bool,
    pub zid_pubkey: Vec<u8>,
    pub status_sig: Vec<u8>,
}

/// Fountain-decode UR parts of a given UR type back to the raw bytes.
fn decode_ur_fountain(ur_parts: Vec<String>, ur_type: &str) -> Result<Vec<u8>, ErrorDisplayed> {
    if ur_parts.is_empty() {
        return Err(ErrorDisplayed::Str {
            s: "No UR parts provided".to_string(),
        });
...
fn hex_arr<const N: usize>(hex_str: &str, what: &str) -> Result<[u8; N], ErrorDisplayed> {
    let bytes = hex::decode(hex_str).map_err(|e| ErrorDisplayed::Str {
        s: format!("invalid {what} hex: {e:?}"),
    })?;
    bytes.try_into().map_err(|_| ErrorDisplayed::Str {
        s: format!("{what} must be {N} bytes"),
    })
}

/// Verify a `ur:zafu-stream` fountain against the pinned update key and the
/// current version/board. Returns the manifest summary for the confirm screen
/// on success; any trust check failure raises. `manifest_sig_hex` is the
/// manifest signature (over fields 1..7) carried by the carrier layer — the
/// frozen wire keeps image_sig inline (frame0 field 8) but does not embed the
/// manifest signature.
pub fn ota_verify_stream(
    ur_parts: Vec<String>,
    manifest_sig_hex: String,
    pinned_pubkey_hex: String,
    current_version: String,
    board: String,
) -> Result<OtaStreamManifestSummary, ErrorDisplayed> {
    ...
    Ok(OtaStreamManifestSummary {
        ...
        payload_size: vs.manifest.payload_size,
        min_version: vs.manifest.min_version,
...
/// Encode a device-signed `ur:zafu-result` (single-frame) as UR string(s).
/// `slot` is b'A' or b'B'. Signs fields 1..5 with the ZID identity key.
pub fn ota_encode_result(
    fw_version: String,
    success: bool,
    slot: u8,
    req_id: Vec<u8>,
    zid_secret_key_hex: String,
) -> Result<Vec<String>, ErrorDisplayed> {
    let sk = hex_arr::<32>(&zid_secret_key_hex, "zid_secret_key")?;
    let req: [u8; 8] = req_id.try_into().map_err(|_| ErrorDisplayed::Str {
        s: "req_id must be 8 bytes".to_string(),
    })?;
...
pub fn ota_decode_result(ur_parts: Vec<String>) -> Result<OtaResultFFI, ErrorDisplayed> {
    ...
    Ok(OtaResultFFI {
        ...
        success: r.success,
        slot: r.slot,
        req_id: r.req_id.to_vec(),
        zid_pubkey: r.zid_pubkey.to_vec(),
        result_sig: r.result_sig.to_vec(),
    })
}
...
pub fn ota_decode_status(ur_parts: Vec<String>) -> Result<OtaStatusFFI, ErrorDisplayed> {
    ...
    Ok(OtaStatusFFI {
        ...
        slot: s.slot,
        successful_boot: s.successful_boot,
        zid_pubkey: s.zid_pubkey.to_vec(),
        status_sig: s.status_sig.to_vec(),
    })
}
