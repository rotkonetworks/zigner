...
/// One release key per seed. The derivation index is fixed - a seed has exactly
/// one release identity, not a family of them. Signatures are untagged and the
/// kernel matches them against the pinned key set, so there is no slot to pick
/// and nothing to keep consistent between exporting the key and signing with it.
const RELEASE_INDEX: u32 = 0;
...
/// This seed's release public key, hex, for baking into `RELEASE_KEY_BYTES`.
/// One key per seed; safe to publish (it is the public half).
pub fn public_key_hex(seed_phrase: &str) -> Result<String, String> {
    let pair = derive_domain_keypair(seed_phrase, RELEASE_DOMAIN, RELEASE_INDEX)?;
...
pub fn sign_request(seed_phrase: &str, prefix: &[u8]) -> Result<(String, String), String> {
...
mod tests {
    ...
    fn verifying_key(seed: &str) -> VerifyingKey {
        let hexed = public_key_hex(seed).unwrap();
    ...
    fn release_keys_are_separate_from_ssh_and_identity() {
        let release = public_key_hex(SEED_A).unwrap();
    ...
    fn distinct_seeds_give_distinct_release_keys() {
        ...
        let a = public_key_hex(SEED_A).unwrap();
        let b = public_key_hex(SEED_B).unwrap();
        let c = public_key_hex(SEED_C).unwrap();
        assert_ne!(a, b);
        assert_ne!(b, c);
        assert_ne!(a, c);
    }
    ...
    fn refuses_anything_that_is_not_a_manifest() {
        for junk in [
            ...
            assert!(
                sign_request(SEED_A, &junk).is_err(),
    ...
    fn refuses_trailing_bytes_after_the_signed_region() {
        ...
        assert!(sign_request(SEED_A, &p).is_err());
    }
    ...
    fn two_devices_produce_a_package_the_kernel_accepts() {
        ...
        let (_, sig_a) = sign_request(SEED_A, &p).unwrap();
        let (_, sig_c) = sign_request(SEED_C, &p).unwrap();

        let package = manifest::assemble_package(
            &p,
            &[
                hex::decode(sig_a).unwrap().try_into().unwrap(),
