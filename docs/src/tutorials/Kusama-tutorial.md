# Zigner tutorial with Polkadot-js apps

This tutorial will walk you through setting up a Kusama account with the Zigner Android or iOS App and then use this account together with [Polkadot-js apps](https://polkadot.js.org/apps/) to see your balance and transfer funds or perform any extrinsic from this account.

* Notice: The UI may be variant for different versions, but the functionalities are the same in `v4` version.

## Summary

- [1. Get Zigner mobile application](#1-get-zigner-mobile-application)
...
- [3. Add Zigner's account to Polkadot-js apps](#3-add-zigners-account-to-polkadot-js-apps)
- [4. Sign a transaction](#4-sign-a-transaction)

## 1. Get Zigner mobile application

### Device security

Zigner is meant to be used offline. The mobile device used to run Zigner will hold valuable information that needs to be kept securely stored. It is therefore advised to:

- Get a Zigner dedicated mobile device.
...
- Once Zigner has been installed, enable airplane mode and make sure to switch off Wifi, Bluetooth or any connection ability of the device.
...
### Install Zigner mobile application

Install Zigner from [GitHub Releases](https://github.com/rotkonetworks/zigner/releases). Verify the APK signature and checksum.
...
If you already have an account created with either Zigner or any other wallet, you can recover it by doing so:
...
## 3. Add Zigner's account to Polkadot-js apps

To be able to follow this tutorial and interact with the Blockchain from a freshly created account on Zigner, you will need to get some KSMs on this account first. Polkadot-js apps allows you to manage your Zigner account seamlessly.
...
- It will ask for the webcam permission for you to scan the Zigner's account QR code, accept it.
- On Zigner, choose on the account you want to copy the address of.
...
Assuming that your Zigner account now has funds, you will be able to send some funds securely to anyone, without transferring your private key, and without needing any internet connection on your mobile phone.
...
You will now be presented with a QR code that represents the transaction. Since this transaction is sending funds from your Zigner mobile app account, only this account (sitting on your phone) can sign and authorize this transaction. This is what we'll do in the next steps:

- From the Zigner account overview, tap the scan button on the top right and scan the QR code presented by the Polkadot-js apps website.
...
Your phone has now *signed the transaction offline* using your Zigner account private key. The QR code that is now displayed on your phone represents a signed transaction that can be broadcasted. We will do this in the next steps:
